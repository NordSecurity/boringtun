...
use crate::noise::packet::{HandshakeInit, HandshakeResponse, ParsedPacket};
...
impl RateLimiter {
    ...
    pub fn verify_packet<'a, 'b>(
        ...
        src: &'a [u8],
        dst: &'b mut [u8],
    ) -> Result<ParsedPacket<'a>, TunnResult<'b>> {
        let packet = parse_incoming_packet(src)?;

        // Verify and rate limit handshake messages only
        if let ParsedPacket::HandshakeInit(HandshakeInit { sender_idx, .. })
        | ParsedPacket::HandshakeResponse(HandshakeResponse { sender_idx, .. }) = packet
        {
