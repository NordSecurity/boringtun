...
impl NoiseParams {
    ...
    // Should be pared with format_*
    pub fn receive_handshake_initiation(
        &self,
        last_handshake_timestamp: &mut Tai64N,
        ...
        if !timestamp.after(last_handshake_timestamp) {
            // Possibly a replay
        ...
        *last_handshake_timestamp = timestamp;
        ...
        // if Err(e) = self.format_handshake_response(&mut packet) {
        //     return Err((packet, e));
        // }
        Ok(InitRecvState {
            chaining_key,
    ...
    /// Should be paired with receive_*
    pub fn format_handshake_response(
        ...
        //     return Err(WireGuardError::DestinationBufferTooSmall);
        // }
        let dst = packet.reset().set_data(HANDSHAKE_RESP_SZ).data_mut();
        ...
        sender_index.copy_from_slice(&index.to_le_bytes());
